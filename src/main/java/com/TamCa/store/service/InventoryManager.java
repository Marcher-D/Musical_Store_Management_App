...
public class InventoryManager {
    ...
    // Trả về Role (Manager/Staff) nếu đúng, trả về null nếu sai
    public String checkLogin(String username, String password) {
        // Query kiểm tra tài khoản và mật khẩu
        ...
        try (Connection conn = getConnection(true);
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    return rs.getString("role"); // Đăng nhập thành công
                }
            }
        } catch (SQLException e) {
            System.err.println("Lỗi DB khi kiểm tra đăng nhập: " + e.getMessage());
        }
